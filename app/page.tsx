...
const liveCopy: Record<Lang, { badge: string; title: string; listen: string; next: string }> = {
  de: { badge: "JETZT VERÖFFENTLICHT", title: "Persian Edition ist live", listen: "Jetzt anhören", next: "Nächste Edition" },
  en: { badge: "OUT NOW", title: "Persian Edition is live", listen: "Listen now", next: "Next edition" },
  fr: { badge: "DISPONIBLE MAINTENANT", title: "La Persian Edition est en ligne", listen: "Écouter maintenant", next: "Prochaine édition" },
  es: { badge: "YA DISPONIBLE", title: "La Persian Edition ya está disponible", listen: "Escuchar ahora", next: "Próxima edición" },
  bs: { badge: "SADA DOSTUPNO", title: "Persian Edition je objavljena", listen: "Slušaj sada", next: "Sljedeće izdanje" },
};
...
export default function Home() {
  ...
  return <main>
    ...
    <section className="releaseStory"><div><p className="sectionNo">02 — THE RELEASE</p><h2>{t.releaseTitle}</h2><p>{t.releaseCopy}</p></div><div className="platforms" aria-label="Music platforms">{["Spotify", "Apple Music", "Amazon Music", "YouTube Music", "Deezer"].map(platform => <a href={PERSIAN_EDITION_URL} target="_blank" rel="noopener noreferrer" key={platform}>{platform}<small>{live.listen} ↗</small></a>)}</div></section>
